...
{
    public Task<List<AnimeEntity>?> GetUserAnimeListAsync(CancellationToken ct = default)
    {
        Log.Warning("Shikimori full-list sync is disabled; skipping destructive local mirror update.");
        return Task.FromResult<List<AnimeEntity>?>(null);
    }
    ...
    public Task<List<AnimeEntity>?> GetUserMangaListAsync(CancellationToken ct = default)
    {
