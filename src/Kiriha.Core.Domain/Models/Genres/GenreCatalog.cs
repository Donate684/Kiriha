...
public sealed record GenreDefinition(
    string Key,
    string EnglishName,
    string RussianName,
    string[]? Aliases = null)
...
public static class GenreCatalog
{
    ...
    static GenreCatalog()
    {
        var lookup = new Dictionary<string, GenreDefinition>(StringComparer.OrdinalIgnoreCase);
        var byKey = new Dictionary<string, GenreDefinition>(StringComparer.OrdinalIgnoreCase);
    ...
    private static void Register(Dictionary<string, GenreDefinition> map, string key, GenreDefinition def)
    {
    ...
    public static IReadOnlyList<GenreDefinition> All => Definitions;
    ...
    public static bool TryFindGenre(string? queryOrToken, out GenreDefinition? definition)
    {
        definition = null;
    ...
    public static GenreDefinition? FindByKey(string key)
    {
        return ByKeyMap.TryGetValue(key, out var def) ? def : null;
    }

    public static IReadOnlyList<GenreDefinition> SearchGenres(string? query, int maxResults = 8)
    {
