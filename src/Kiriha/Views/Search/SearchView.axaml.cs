...
public partial class SearchView : UserControl
{
    ...
    {
        if (e.Container.DataContext is Kiriha.Core.Abstractions.Models.Entities.AnimeEntity item && DataContext is SearchViewModel vm)
        {
    ...
    private async void Poster_DoubleTapped(object? sender, Avalonia.Input.TappedEventArgs e)
    {
        ...
        try
        {
            if (sender is Control c && c.DataContext is Kiriha.Core.Abstractions.Models.Entities.AnimeEntity item && DataContext is SearchViewModel vm)
            {
