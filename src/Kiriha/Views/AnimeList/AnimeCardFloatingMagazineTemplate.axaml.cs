...
{
    public partial class AnimeCardFloatingMagazineTemplate : ResourceDictionary
    {
        public AnimeCardFloatingMagazineTemplate()
        {
            InitializeComponent();
        }
        ...
        private async void Poster_DoubleTapped(object? sender, TappedEventArgs e)
        {
            try
            {
                if (sender is Control c && c.DataContext is Kiriha.Core.Abstractions.Models.Entities.AnimeEntity item)
                {
        ...
        {
            if (sender is MenuFlyout flyout && flyout.Target is Control target && target.DataContext is Kiriha.Core.Abstractions.Models.Entities.AnimeEntity item)
            {
