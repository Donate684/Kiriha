...
namespace Kiriha.Views.AnimeList
{
    public partial class AnimeCardFloatingMagazineTemplate : ResourceDictionary
    {
        ...
        private async void Poster_DoubleTapped(object? sender, TappedEventArgs e)
        {
            try
            {
                if (sender is Control c && c.DataContext is Models.Entities.AnimeEntity item)
                {
        ...
        {
            if (sender is MenuFlyout flyout && flyout.Target is Control target && target.DataContext is Models.Entities.AnimeEntity item)
            {
