...
internal static class SeasonalFilterEngine
{
    ...
    private static IEnumerable<AnimeEntity> ApplyBaseFilters(SeasonalFilterRequest request)
    {
...
{
    private readonly Dictionary<string, List<AnimeEntity>> _items;

    private SeasonalCategoryBuckets(Dictionary<string, List<AnimeEntity>> items)
    {
        _items = items;
    }

    public static SeasonalCategoryBuckets Build(IEnumerable<AnimeEntity> source, int currentYear, string currentSeason)
    {
        var buckets = new Dictionary<string, List<AnimeEntity>>
        {
            ["New"] = new(),
            ["Continuing"] = new(),
            ["Movies"] = new(),
    ...
    public IReadOnlyList<AnimeEntity> GetItems(string category) => _items.TryGetValue(category, out var items)
        ? items
        : _items["Other"];
